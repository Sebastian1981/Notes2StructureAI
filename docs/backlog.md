...
| US-031 | Als Nutzer möchte ich alle regulären Ansichten aus einer vollständigen Analyse erhalten, damit ich nicht für Reinschrift, Notizen und das passende Diagramm mehrfach bezahlen oder warten muss. | Die primäre UI-Aktion verwendet `full` mit automatischer Klassifikation und zeigt Reinschrift, Notizen, Diagramm, Mermaid und JSON aus demselben Ergebnis. | `Erledigt` |
...
| US-035 | Als Nutzer möchte ich das vorhandene Ergebnis optional als anderen Diagrammtyp interpretieren, damit ich Varianten prüfen kann, ohne das Bild erneut hochzuladen. | Jede Schaltfläche kennzeichnet den zusätzlichen Provideraufruf; übertragen wird nur validiertes Analyse-JSON. Text und Notizen bleiben unverändert, fehlende Evidenz führt zu keinem erfundenen Diagramm. | `Erledigt` |
